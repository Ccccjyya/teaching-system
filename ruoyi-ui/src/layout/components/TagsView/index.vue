...
export default {
  ...
  computed: {
    visitedViews() {
      return this.filterRoleHomeTags(this.$store.state.tagsView.visitedViews)
    },
    routes() {
  ...
  methods: {
    isTeacherRole() {
      ...
      return roles.includes('teacher') || this.$store.getters.identity === 'teacher'
    },
    isHomeTag(tag, type) {
      const tagPath = tag.path || tag.fullPath || ''
      const tagName = tag.name || ''
      if (type === 'admin') {
        return tagPath === '/dashboard' || tagName === 'Dashboard'
      }
      if (type === 'student') {
        return tagPath === '/studentIndex' || tagName === 'StudentIndex'
      }
      return tagPath === '/teacherIndex' || tagPath === '/teacher' || tagName === 'TeacherIndex' || tagName === 'TeacherHome'
    },
    isRoleHomeTag(tag, type) {
      const tagPath = tag.path || tag.fullPath || ''
      const tagName = tag.name || ''
      if (type === 'admin') {
        return tagPath === '/dashboard' || tagName === 'Dashboard'
      ...
      return tagPath === '/teacherIndex' || tagName === 'TeacherIndex'
    },
    filterRoleHomeTags(tags) {
      const roles = this.$store.getters.roles || []
      const isStudent = roles.includes('student') || this.$store.getters.identity === 'student'
      const isAdmin = roles.includes('admin')
      const isTeacher = this.isTeacherRole()
      return (tags || []).filter(tag => {
        const isAdminHome = this.isHomeTag(tag, 'admin')
        const isStudentHome = this.isHomeTag(tag, 'student')
        const isTeacherHome = this.isHomeTag(tag, 'teacher')
        const isHome = isAdminHome || isStudentHome || isTeacherHome
        if (isTeacher) return !isHome || this.isRoleHomeTag(tag, 'teacher')
        if (isStudent) return !isHome || this.isRoleHomeTag(tag, 'student')
        if (isAdmin) return !isHome || this.isRoleHomeTag(tag, 'admin')
        return true
      })
    },
    filterRoleAffixTags(tags) {
      return this.filterRoleHomeTags(tags)
    },
    handleKeyDown(event) {
    ...
    isFirstView() {
      try {
        ...
        return tag.fullPath === this.roleHomePath() || tag.fullPath === this.visitedViews[1].fullPath
      } catch (err) {
        return false
      }
    },
    roleHomePath() {
      const roles = this.$store.getters.roles || []
      if (this.isTeacherRole()) return '/teacherIndex'
      if (roles.includes('student') || this.$store.getters.identity === 'student') return '/studentIndex'
      return '/dashboard'
    },
    isLastView() {
      try {
    ...
    initTags() {
      ...
      const affixTags = this.affixTags = this.filterRoleAffixTags(this.filterAffixTags(this.routes))
    ...
    closeSelectedTag(view) {
      this.$tab.closePage(view).then(({ visitedViews }) => {
        if (this.isActive(view)) {
          this.toLastView(this.filterRoleHomeTags(visitedViews), view)
        }
      })
    },
    closeRightTags() {
      this.$tab.closeRightPage(this.selectedTag).then(visitedViews => {
        const roleViews = this.filterRoleHomeTags(visitedViews)
        if (!roleViews.find(i => i.fullPath === this.$route.fullPath)) {
          this.toLastView(roleViews)
        }
      })
    },
    closeLeftTags() {
