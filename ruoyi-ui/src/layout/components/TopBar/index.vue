...
export default {
  ...
  computed: {
    ...
    topMenus() {
      let routes = this.$store.state.permission.sidebarRouters.filter((f) => !f.hidden)
      if (auth.hasRole('admin')) {
        routes = routes.filter(route => {
          const path = (route.path || '').replace(/^\/+/, '')
          return path !== 'studentIndex' && path !== 'teacherIndex' && path !== 'teacher'
        })
      } else if (auth.hasRole('student')) {
        routes = routes.filter(route => {
          const path = (route.path || '').replace(/^\/+/, '')
          return path !== 'dashboard' && path !== 'teacherIndex' && path !== 'teacher'
        })
      } else if (auth.hasRole('teacher') || this.$store.getters.identity === 'teacher') {
        routes = routes.filter(route => {
          const path = (route.path || '').replace(/^\/+/, '')
          return path !== 'dashboard' && path !== 'studentIndex' && path !== 'teacher'
        })
      }
      return routes.slice(0, this.visibleNumber)
    },
    moreRoutes() {
      let routes = this.$store.state.permission.sidebarRouters.filter((f) => !f.hidden)
      if (auth.hasRole('admin')) {
      ...
      return routes.slice(this.visibleNumber, routes.length - this.visibleNumber)
    },
    // 默认激活的菜单
    activeMenu() {
